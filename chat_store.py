...
与会话元数据（title / 时间戳）；LLM 上下文只取最近
``HISTORY_MAX_MESSAGES`` 条，完整回放受 ``HISTORY_MAX_REPLAY`` 条数限制。
...
def _new_session_record() -> dict[str, Any]:
    """创建空的会话记录（title / 时间戳 / 消息列表）。"""
    now = _now_iso()
    return {"title": "", "created_at": now, "updated_at": now, "messages": []}
...
class HistoryStore(ABC):
    ...
    @abstractmethod
    def append(self, session_id: str, question: str, answer_payload: dict[str, Any]) -> None:
        """把一轮问答（含完整回放 payload）追加到会话历史，并裁剪到上限。"""

    @abstractmethod
    def list_sessions(self) -> list[dict[str, Any]]:
        """返回全部会话元数据（含 session_id/title/时间/消息数），按最近更新倒序。"""
